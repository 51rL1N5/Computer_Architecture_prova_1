# C�digo em assembly do histograma

.data

colunas: .word 64 # N�mero de colunas da imagem
linhas:  .word 81 # N�mero de linhas da imagem
...
# Se houver algum dado que considere importante para seu c�digo,
# pode colocar aqui.
...
endl:   .asciiz "\n"
.text
# �rea para o c�digo assembly

main:
    la $s0, imagem # $s0 <- endereço[imagem]
    la $s1, H      # $s1 <- endereço[H]

    add $t0, $zero, $s1 # $t0 <- $s1
    add $t2, $zero, $s0 # $t2 <- $s0

    li $t8, 0           # inicialização do indice
    li $t9, 256         # definição do limite de iterações do loop
initH:

    sll $t1, $t8, 2     # devido a trabalharmos com bytes, temos de multiplicar por 4 o indice
    add $t1, $t0, $t1   # incrementa o (indice * 4) no endereço base do vetor

    sw $zero, 0($t1)    # H[i] <- 0

    addi $t8, $t8, 1    # incrementa o indice $t8

    bne $t8, $t9, initH # checar condição de Loop

    li $t8, 0           # reinicialização do indice
    li $t9, 5184        # definição do limite de iterações do loop

#t3 para percorrer a imagem
#t6 para percorrer o H
#loop t8 to t9  (0 to 5184)
assign:
    sll $t3, $t8, 2     # tratar indice
    add $t3, $s0, $t3   # $t3 <-  endereço base + (indice * 4)

    lw $t4, 0($t3)      # $t4 <- Imagem[endereço base + (indice * 4)]

    sll $t4, $t4, 2     # tratar índice pois, $t4 será o índice de H

    add $t6, $s1, $t4   # $t6 <- $s1 (endereço base de H) + $t4

    lw $t5, 0($t6)      # $t5 <- H[$t6]
    addi $t5, $t5, 1    # $t5 <- $t5 + 1
    sw $t5, 0($t6)      # H[$t6] <- $t5

    addi $t8, $t8, 1     # incrementa o índice $t8

    bne $t8, $t9, assign # checar condição de Loop

    li $t8, 0            # reinicialização do indice
    li $t9, 256          # definição do limite de iterações do loop
plot:
    sll $t1, $t8, 2      # tratar indice
    add $t1, $s1, $t1    # $t1 <- endereço base + (indice * 4)

    li $v0, 1            # print integer
    add $a0, $t8, $zero  # $a0 <- $t8
    syscall
    ...
    addi $t8, $t8, 1     # incrementar índice $t8

    bne $t8, $t9, plot   # checar condição de loop
